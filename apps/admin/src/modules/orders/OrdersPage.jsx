  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          <p className="text-sm font-bold text-slate-500">Sales / Orders</p>
          <h1 className="mt-1 text-2xl font-black">Orders</h1>
          <p className="mt-1 text-sm font-semibold text-slate-500">
            Manage customer orders, vendor fulfillment and delivery status.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-panel-line dark:divide-panel-darkLine">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-white/5">
                  ...
                  <td className="px-5 py-4 text-sm font-semibold">{order.payment}</td>
                  <td className="px-5 py-4 text-sm font-semibold">{order.delivery}</td>
                  <td className="px-5 py-4">
                    <OrderStatusBadge status={order.status} />
                  </td>
                  <td className="px-5 py-4 text-sm font-semibold">{order.date}</td>
